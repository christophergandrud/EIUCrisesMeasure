...
To estimate marginal or `off-trend' government debt changes in response to financial crises we ran a series of regressions and focused on the resulting residuals. Table \ref{residuals} shows the two step process. First, we tried to identify off-trend costs associated with general government distress. To do this, we used a partial adjustment model with lagged central government debt to control for serial autocorrelation as well as countries' output gaps. The initial model predicting debt with the output gap includes country fixed effects. The debt variable is from the World Bank's Development Indicators.\footnote{\url{http://data.worldbank.org/data-catalog/world-development-indicators}. Accessed June 2015. It was originally recorded as a percentage of the same-year GDP. To strip out GDP changes, we rebased
