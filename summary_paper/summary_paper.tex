...
Most recent research relied on one of two measures of financial crisis-- \cite{Reinhart2009} or \cite{laeven2013}--or a predecessor. These generally use post-hoc assessments of banking crisis as a second-best alternative to real-time information. However, this presents clear problems. Chiefly, using such measures creates clear selection bias as stress that politicians responded to effectively will not be selected. In addition, these measures are typically binary and so give no indication of stress intensity. The measures are also at gross intervals, typically yearly, prohibiting sub-annual analysis.
...
information on when a country is facing a financial crisis--\cite{laeven2013} and \cite{Reinhart2009} or a direct predecessor. For a summary see \cite{GandrudHallerberg2015} as well as tables \ref{LitRevTable} and \ref{LitRevTable2}. Knowing when crises
...
\cite{GandrudHallerberg2015} find that there are significant
difference in crisis timing between different versions of the \cite{laeven2013} data. Crises are also identified \emph{post hoc} by researchers who know
...
Statistics, which Copelovitch, Gandrud, and Hallerberg (2015) indicate could be endogenous to political events.

\cite{Romer2015} aimed to address this issue by manually
classifying 24 countries on a 16 point scale capturing the cost of
...
present and future economic conditions. They are also a channel through which this information is disseminated to public
...
continuous more-stressed/less stressed spectrum for as many country-months as possible. Therefore, we clearly need an
efficient way to summarize the vast quantity of information in the EIU
reports along such a spectrum. To do this we first collected and processed the texts. We then used kernel principal component analysis to summarize the texts into a
...
not just their financial system. Our first step was to select the
portions of the EIU texts that contained relevant information about
countries' banking and financial systems. We automatically collected and the parsed
