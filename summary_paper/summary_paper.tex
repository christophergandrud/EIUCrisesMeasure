...
The lines in figures \ref{compare_1} \ref{compare_2} show the results of
the kernel PCA analysis for a selection of countries. We use the first
principal component throughout the paper. Similar plots for all
countries in the analysis are available in the Appendix. Before
...
rescaled the index so that it would be between zero and one. This eases
interpretation and comparability to other measures. Then we slightly
smoothed the results by taking a two period--usually two months--moving
average.
...
\subsection{Representation and~comparison to other crisis
measures}\label{representation-andcomparison-to-other-crisis-measures}
...
The shadded boxes in figures \ref{compare_1} and \ref{compare_2} show
the periods where Laeven and Valencia (2013) and Reinhart and Rogoff
(2009 \^{}reinhart-address) classify there as being a banking crisis.
