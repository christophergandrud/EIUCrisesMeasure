...
documents that involve the words `slowing credit' and `expanding
credit'. They share the five character kernels `credit' and `ingcr', but
differ on `wingc' and `dingc'. Using Lodhi et al. (2002) we can
summarize the similarity of these documents using with the frequency
distribution of five-length strings that they have in
common--two--standardized by document length. We can find these pairs
...
analysis.\footnote{We conducted kernel PCA with the R package
  \texttt{kernlab} (Karatzoglou et al. 2004).}
...
Valencia (2013) as well as Romer and Romer's
\href{mailto:-@Romer2014}{\nolinkurl{-@Romer2014}} continuous measure.
